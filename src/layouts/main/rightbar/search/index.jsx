...
export default function Search() {
    ...
    return (
        <div
            ref={ref}
            className="min-h-[32px] h-[53px] mb-3 flex items-center relative sticky top-0 bg-black z-10">
            <label className="h-[43px] rounded-full bg-[#202327] w-full relative group border border-transparent focus-within:bg-black focus-within:border-[#1d9bf0]">
                ...
                {(query && focus) && (
                    <button
                        className="w-[22px] h-[22px] rounded-full bg-[#1d9bf0] flex items-center justify-center text-black min-w-[22px] absolute -translate-y-1/2 top-1/2 right-3"
                        type="button"
                        onClick={() => setQuery('')}
                    >
            ...
            {focus && (
                <div className="absolute w-[350px] -left-px top-full -translate-y-1  bg-black text-center shadow-box max-h-[calc(80vh-53px)] rounded-lg min-h-[100px]">
                    <p className="p-3 pt-5 text-[#71757b] leading-5">
